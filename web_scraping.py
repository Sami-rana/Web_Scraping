...
class HuggingFaceScraper:
    ...
    def model_addresses(self, href_values, cache):
        ...
        for url in href_values:
            ...
            if response_text is not None:
                ...
                # Find all elements with class="block p-2"           <a class="block p-2"
                # href="/stabilityai/control-lora">
                elements = soup.find_all(class_='block p-2')
    ...
    def fetch_huggingface_model_urls(self, href_values, cache):
        ...
        for url in href_values:
            ...
            if response_text is not None:
                ...
                # Extract the href attribute values from these elements
                model_url.extend(
                    [f'https://huggingface.co{element["href"]}' for element in elements if 'href' in element.attrs])
            ...
            if response_text is not None:
                ...
                if main_element:
                    ...
                    header_element = main_element.find('header',
                                                       class_='from-gray-50-to-white border-b border-gray-100 '
                                                              'bg-gradient-to-t via-white dark:via-gray-950 pt-6 '
                                                              'sm:pt-9')
                    if header_element:
                        # Remove the header element from the DOM
                        header_element.extract()

                        # Find and exclude the element with class "pt-8 border-gray-100 md:col-span-5 pt-6 md:pb-24
                        # md:pl-6 md:border-l order-first md:order-none"
                    div_element = main_element.find('div',
                                                    class_='pt-8 border-gray-100 md:col-span-5 pt-6 md:pb-24 md:pl-6 '
                                                           'md:border-l order-first md:order-none')
                    if div_element:
                        # Remove the div element from the DOM
                        div_element.extract()

                        # Find and exclude the <section> element with the same class as the div element
                    section_element = main_element.find('section',
                                                        class_='pt-8 border-gray-100 md:col-span-5 pt-6 md:pb-24 '
                                                               'md:pl-6 md:border-l order-first md:order-none')
                    if section_element:
                        # Remove the section element from the DOM
                        section_element.extract()

                        # Find and exclude the <a> element with class "btn mb-6 text-sm text-gray-600 md:absolute
                        # md:-right-6 md:top-0 md:mb-0 md:rounded-r-none md:rounded-t-none md:border-r-0
                        # md:border-t-0 md:border-gray-100 md:bg-none"
                    a_element = main_element.find('a',
                                                  class_='btn mb-6 text-sm text-gray-600 md:absolute md:-right-6 '
                                                         'md:top-0 md:mb-0 md:rounded-r-none md:rounded-t-none '
                                                         'md:border-r-0 md:border-t-0 md:border-gray-100 md:bg-none')
                    if a_element:
    ...
    def scrape_category_tags(self, model_urls, cache):
        ...
        for url in model_urls:
            ...
            if response_text is not None:
                ...
                category_items = {'Task': [], 'Library': [], 'Language': [], 'Others': [], 'Arxiv': [], 'License': [],
                                  'Dataset': []}
    ...
    def save_to_csv(self, data, csv_file_path):
        ...
        try:
            ...
            with open(csv_file_path, mode='w', newline='', encoding='utf-8') as csv_file:
                ...
                # Write header row
                writer.writerow(
                    ["Sr.no", "ModelName", "ModelRepo", "ModelAddress", "ModelUrl", "Tasks", "Libraries", "Dataset",
                     "Languages", "Other", "Arxiv", "Licenses", "Github Links", "Body"])
    ...
    def run(self):
        ...
        rows = zip(range(1, len(model_names) + 1), model_names, model_repo, model_addresses, model_urls, task_data,
                   library_data, dataset_data, language_data, others_data, arxiv_data, license_data,
                   scrape_github_links, extract_model_cards)

        # Specify the CSV file path
        csv_file_path = '/home/sami/PycharmProject/Hugging_Face/Scrap.csv'
